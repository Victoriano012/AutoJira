...
- **Projects are folders.** A new project creates a folder under `~/Documents/personal` (override with `AUTOPROJECT_HOME`); the agent works right in it. Import any existing folder from the picker — its `.autoproject` state is adopted, or created on the spot. Everything is autosaved to `<workspace>/.autoproject/project.json`; no database, no login.
